...
import { useGetAllCasesWithQueryQuery, useGetAllcasesProductsQuery } from "features/api/caseApi";
...
const CaseStore: React.FC<CaseStoreProps> = ({
  categoriesState,
  ...
  const { data: caseProducts, isLoading } = useGetAllCasesWithQueryQuery(queries, {
    refetchOnMountOrArgChange: true,
  });

  const metaData = caseProducts?.meta;
  ...
  return (
    <div>
      ...
      ) : caseProducts?.result && caseProducts.result.length > 0 ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10 mt-8">
          {caseProducts.result.map((item: any, index: any) => (
            <div
              ...
              <div className="space-y-4 px-3 pt-5 pb-2.5">
                <div className="flex justify-between items-start w-full ">
                  <div>
                    ...
                    >
                      {item?.product_name?.length >= 18 ? item?.product_name?.slice(0, 18) + "..." : item?.product_name}

                    </h2>
                  </div>
                  <Prices price={item?.case_high_price} />
                </div>
